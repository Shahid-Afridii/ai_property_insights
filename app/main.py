...
from app.ai_predict import predict_future
...
def future_predict_api(data: PropertyInput):
    ...
    match_result = run_query(street, postcode, area, outward, micro, building_type)

    # 🔥 SAME LOGIC AS YOUR MATCHING
    if match_result["exactMatches"] or match_result["streetMatches"]:
        rows_data = match_result["exactMatches"] + match_result["streetMatches"]
    else:
        rows_data = match_result["areaMatches"]

    # 🔥 AI forecast
    forecast = predict_future(rows_data)

    return {
        "success": True,
        "matchedBy": match_result["matchedBy"],
        "matchedCount": len(rows_data),
        "forecast": forecast
    }
